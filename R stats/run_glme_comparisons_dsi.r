run_glme_comparisons_dsi <- function(file_path, sheet_name) {
  ...
  result_full <- fit_and_compare(DSI ~ Genotype * Area + (1 | ID), T_B)

  # Subset: crtx_sup
  T_B_sup <- filter(T_B, Area == "crtx_sup")
  result_sup <- fit_and_compare(DSI ~ Genotype + (1 | ID), T_B_sup)

  # Subset: crtx_deep
  T_B_deep <- filter(T_B, Area == "crtx_deep")
  result_deep <- fit_and_compare(DSI ~ Genotype + (1 | ID), T_B_deep)
