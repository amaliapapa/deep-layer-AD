run_glme_comparisons_br <- function(file_path, sheet_name) {
  # Load libraries
  library(lme4)
  library(tidyverse)
  library(readxl)
  ...
  # Run comparisons
  result_full <- fit_and_compare(BR ~ Genotype * Area + (1 | ID), T_B)

  T_B_sup <- filter(T_B, Area == "crtx_sup")
  result_sup <- fit_and_compare(BR ~ Genotype + (1 | ID), T_B_sup)

  T_B_deep <- filter(T_B, Area == "crtx_deep")
  result_deep <- fit_and_compare(BR ~ Genotype + (1 | ID), T_B_deep)
